...
class InputInfo(object):
    ...
    def queries(self, dataset):
        ...
        if len(self.run) is not 0:
            return ["file {0}={1} run={2} site=T0_CH_CERN_MSS".format(query_by, query_source, query_run) for query_run in self.run]
            #return ["file {0}={1} run={2} ".format(query_by, query_source, query_run) for query_run in self.run]
        else:
            return ["file {0}={1} site=T0_CH_CERN_MSS".format(query_by, query_source)]
